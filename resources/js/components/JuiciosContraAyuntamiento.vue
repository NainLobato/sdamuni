        ...
        <b-collapse id="colapsable" v-model="colapsableEstado">
            <b-card-body align="left">
                    <div class="row">
                        ...
                        <div class="col-md-12">
                            <div class="form-group">
                                <label for="monto">Importe monetario que puede generarse con el fallo en contra del ayuntamiento:</label>
                                <input type="number" id="monto" name="monto" class="form-control" placeholder="Ingrese monto" data-vv-as="monto" v-validate="'required|max:20'">
                                <div class="invalid-feedback" v-if="errors.has('monto')">{{ errors.first('monto') }}</div>
                            </div>
                        </div>
                        <div class="col-md-12">
                            <div class="form-group">
                                <label for="denunciante">Nombre de la persona que inicio el proceso:</label>
                                <input type="text" id="denunciante" name="denunciante" class="form-control" placeholder="Ingrese nombre" data-vv-as="nombre" v-validate="'required|max:200'">
                                <div class="invalid-feedback" v-if="errors.has('denunciante')">{{ errors.first('denunciante') }}</div>
                            </div>
                        </div>
                        <div class="col-md-12">
                            <div class="form-group">
        ...
        methods: {
            ...
            store () {
                this.$validator.validate().then(valid => {
                    if (valid) {
                        axios.post(route('juicio-contra-ayuntamiento.store').template, { juicio: this.juicio })
                        .then( response => {
            ...
            update () {
                this.$validator.validate().then(valid => {
                    if (valid) {
                        axios.post(route('juicio-contra-ayuntamiento.update').template, { juicio: this.juicio })
                        .then( response => {
                            if (response.data.estado == 2) {
                                Vue.swal({
                                title: 'Exito',
            ...
            eliminar (juicio) {
                Vue.swal({
                    title: '¿Estas seguro de eliminar el juicio contra '+juicio.denunciante+'?',
                    ...
                    if (result.value) {
                        axios.post(route('juicio-contra-ayuntamiento.delete').template, {idJuicio: juicio.id})
